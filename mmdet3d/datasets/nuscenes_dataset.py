...
class NuScenesDataset(Det3DDataset):
    ...
    def get_data_info(self, idx):
        ...
        history_tokens = self._get_prev_tokens(cur_token, num_frames=2)
        info['history_tokens'] = history_tokens
        info['history_poses'] = [None] * len(history_tokens) 
        return info

    def _get_prev_tokens(self, token, num_frames):
        """往前找 num_frames 帧（直接从 data_list 索引，避免递归）"""
        tokens = []
        cur_idx = -1
        for idx, info in enumerate(self.data_list):
            if info['token'] == token:
                cur_idx = idx
                break
        ...
        for i in range(1, num_frames + 1):
            ...
            if hist_idx < 0:
                break
            hist_token = self.data_list[hist_idx]['token']
            tokens.append(hist_token)
